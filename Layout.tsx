...
import { Sparkles, BookOpen, Video, Image as ImageIcon, User, Layers, RotateCcw } from 'lucide-react';
...
export const Layout: React.FC<LayoutProps> = ({ children, activeTab, onTabChange, showTabs = false }) => {
  const handleReset = () => {
    if (confirm('Сбросить все данные приложения? Это удалит ваши курсы и прогресс.')) {
      localStorage.clear();
        ...
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {showTabs && onTabChange && (
              <nav className="flex items-center gap-1 bg-slate-800/50 p-1 rounded-xl border border-slate-700/50 mr-2">
                <button
